...
export const Carousel = () => {
    ...
    return (
      <MoveButtonDiv dir={dir}>
        {dir === "left" ? (
          <FaArrowCircleLeft
            size="50"
            onClick={() => dispatch({ type: (idx - 1).toString() })}
          />
        ) : (
          <FaArrowCircleRight
            size="50"
  ...
  return (
    <CarouselContainer ref={ref} isShown={inView}>
      ...
      <CarouselDiv idx={idx}>
        {projectData.map((el: Project, index: number) => (
          <ProjectCard key={index} content={el} idx={index}></ProjectCard>
        ))}
      </CarouselDiv>
      <MoveButton dir="right"></MoveButton>
      <ItemIndexDiv>
        {Array.from(projectData, (el, index) => (
          <ItemIndex
            isSelected={index === idx}
            onClick={() => dispatch({ type: index.toString() })}
          ></ItemIndex>
        ))}
      </ItemIndexDiv>
    </CarouselContainer>
...
const ItemIndex = styled.div<IsSelected>`
  ...
  background-color: ${({ isSelected }) =>
    isSelected ? ({ theme }) => theme.fontColor.light_gray : "rgba(0,0,0,0)"};
